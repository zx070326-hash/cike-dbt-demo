...
function TodayView({ snapshot, modules, onView, onModule, onEma, onNotification }: { snapshot: Snapshot; modules: ProtocolModule[]; onView: (view: AppView) => void; onModule: (id: string) => void; onEma: () => void; onNotification: (id: string) => void }) {
  ...
  return (
    <div className="nssi-page today-view">
      ...
      {snapshot.notifications?.some((item) => !item.readAt) && <section className="notification-stack" aria-label="未读提醒">{snapshot.notifications.filter((item) => !item.readAt).slice(0, 3).map((item) => <button type="button" key={item.id} onClick={() => onNotification(item.id)}><span><strong>{item.title}</strong><small>{item.body}</small></span><Check />标记已读</button>)}</section>}
...
function ModulesView({ snapshot, modules, selectedId, onSelect, token, onRefresh, onSafety }: { snapshot: Snapshot; modules: ProtocolModule[]; selectedId: string | null; onSelect: (id: string | null) => void; token: string; onRefresh: () => Promise<void>; onSafety: () => void }) {
  ...
  useEffect(() => { if (!selectedId) return; let cancelled = false; fetch(`/api/nssi/module?moduleId=${encodeURIComponent(selectedId)}`).then((response) => response.json()).then((value) => { if (!cancelled) setDetail(value as ModuleDetail); }); return () => { cancelled = true; }; }, [selectedId]);
  ...
  return (
    <div className="nssi-page module-detail-view">
      ...
      <details className="module-sources"><summary><BookOpen />查看本节段落级原文索引（{detail.citations.length}）<ChevronRight /></summary><div>{detail.citations.map((citation) => <article key={citation.id}><header><strong>{citation.section}</strong><span>{citation.printedPage ? `书中 ${citation.printedPage} 页` : `PDF ${citation.pdfPage} 页`} · 第 {(citation.paragraphOrdinal ?? 0) + 1} 段</span></header><blockquote>{citation.evidence}</blockquote><small>{citation.paragraphAnchor ?? citation.chunkId} · OCR 派生文本，专业核对时需回看扫描页</small></article>)}</div></details>
    </div>
  );
}
...
function RecordsView({ snapshot, token, onRefresh }: { snapshot: Snapshot; token: string; onRefresh: () => Promise<void> }) {
  ...
  return <div className="nssi-page records-view">
